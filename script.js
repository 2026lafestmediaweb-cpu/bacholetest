...
function buildConnectorPoints(bhX, bhY, targetX, targetY, type, w, h) {
  switch(type) {
    case 'arc-right': {
      const midX = bhX + (targetX - bhX) * 0.55;
      const midY = bhY + (targetY - bhY) * 0.15;
      const mid2X = targetX + 40;
      const mid2Y = targetY - (targetY - bhY) * 0.35;
      return [
        { x: bhX, y: bhY },
        { x: midX, y: midY },
        { x: mid2X, y: mid2Y },
    ...
    case 'zigzag': {
      const seg1X = bhX + (targetX - bhX) * 0.3;
      const seg1Y = bhY + (targetY - bhY) * 0.6;
      const seg2X = bhX + (targetX - bhX) * 0.6;
      const seg2Y = bhY + (targetY - bhY) * 0.25;
      return [
        { x: bhX, y: bhY },
        { x: seg1X, y: seg1Y },
        { x: seg2X, y: seg2Y },
    ...
    case 'curve-s': {
      const s1X = bhX + (targetX - bhX) * 0.25;
      const s1Y = bhY + (targetY - bhY) * 0.4;
      const s2X = bhX + (targetX - bhX) * 0.55;
      const s2Y = bhY + (targetY - bhY) * 0.65;
      const s3X = bhX + (targetX - bhX) * 0.8;
      const s3Y = bhY + (targetY - bhY) * 0.9;
      return [
        { x: bhX, y: bhY },
        { x: s1X, y: s1Y },
        { x: s2X, y: s2Y },
    ...
    case 'step-down': {
      ...
      const d1Y = bhY + (targetY - bhY) * 0.5;
      const d2X = targetX + 30;
      const d2Y = bhY + (targetY - bhY) * 0.7;
      return [
        { x: bhX, y: bhY },
        { x: d1X, y: d1Y },
        { x: d2X, y: d2Y },
...
function animate() {
  ...
  const bhX = (tempVec.x * 0.5 + 0.5) * window.innerWidth;
  const bhY = (-tempVec.y * 0.5 + 0.5) * window.innerHeight;
  ...
  linkElements.forEach((item, i) => {
    ...
    const points = buildConnectorPoints(
      bhX, bhY, targetX, targetY,
      item.link.connectorType,
